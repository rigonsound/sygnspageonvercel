...
export function ModuleIcons() {
  return (
    <motion.div
      ...
      style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '1rem', maxWidth: '56rem', margin: '0 auto', overflowX: 'auto', padding: '0 1rem', scrollbarWidth: 'none' }}
    >
      {icons.map((icon, index) => (
        ...
        >
          <img src={icon} alt={`Module icon ${index + 1}`} style={{ width: '100%', height: '100%', objectFit: 'contain', opacity: 0.6 }} />
        </motion.div>
      ))}
    </motion.div>
  )
